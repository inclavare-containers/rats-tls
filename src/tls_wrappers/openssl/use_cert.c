...
tls_wrapper_err_t openssl_tls_use_cert(tls_wrapper_ctx_t *ctx, uint8_t *cert_buf, size_t cert_len)
{
	RTLS_DEBUG("ctx: %p, cert_buf: %p, cert_len: %zu\n", ctx, cert_buf, cert_len);

	if (!ctx || !cert_buf || !cert_len)
		return TLS_WRAPPER_ERR_INVALID;

	openssl_ctx_t *ssl_ctx = (openssl_ctx_t *)ctx->tls_private;
	int ret = SSL_CTX_use_certificate_ASN1(ssl_ctx->sctx, cert_len, cert_buf);
	if (ret != SSL_SUCCESS) {
